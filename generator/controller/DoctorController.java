...
public class DoctorController {
    ...
    public BaseResponse<Boolean> deleteDoctor(@RequestBody DeleteRequest deleteRequest, HttpServletRequest request) {
        ...
        // 仅本人或管理员可删除
        if (!oldDoctor.getUserId().equals(user.getId()) && !userService.isAdmin(request)) {
    ...
    public BaseResponse<Page<DoctorVO>> listMyDoctorVOByPage(@RequestBody DoctorQueryRequest doctorQueryRequest,
        ...
        // 补充查询条件，只查询当前登录用户的数据
        User loginUser = userService.getLoginUser(request);
        doctorQueryRequest.setUserId(loginUser.getId());
        long current = doctorQueryRequest.getCurrent();
        long size = doctorQueryRequest.getPageSize();
        // 限制爬虫
        ThrowUtils.throwIf(size > 20, ErrorCode.PARAMS_ERROR);
     ...
     */
    @PostMapping("/edit")
    public BaseResponse<Boolean> editDoctor(@RequestBody DoctorEditRequest doctorEditRequest, HttpServletRequest request) {
        if (doctorEditRequest == null || doctorEditRequest.getId() <= 0) {
            throw new BusinessException(ErrorCode.PARAMS_ERROR);
        }
        // todo 在此处将实体类和 DTO 进行转换
        Doctor doctor = new Doctor();
        BeanUtils.copyProperties(doctorEditRequest, doctor);
        // 数据校验
        doctorService.validDoctor(doctor, false);
        User loginUser = userService.getLoginUser(request);
        // 判断是否存在
        long id = doctorEditRequest.getId();
        Doctor oldDoctor = doctorService.getById(id);
        ThrowUtils.throwIf(oldDoctor == null, ErrorCode.NOT_FOUND_ERROR);
        // 仅本人或管理员可编辑
        if (!oldDoctor.getUserId().equals(loginUser.getId()) && !userService.isAdmin(loginUser)) {
            throw new BusinessException(ErrorCode.NO_AUTH_ERROR);
        }
        // 操作数据库
        boolean result = doctorService.updateById(doctor);
