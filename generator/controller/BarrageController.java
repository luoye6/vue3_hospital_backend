...
public class BarrageController {
     ...
     * 编辑弹幕（给用户使用）
     *
     * @param barrageEditRequest
     * @param request
     * @return
     */
    @PostMapping("/edit")
    public BaseResponse<Boolean> editBarrage(@RequestBody BarrageEditRequest barrageEditRequest, HttpServletRequest request) {
        if (barrageEditRequest == null || barrageEditRequest.getId() <= 0) {
            throw new BusinessException(ErrorCode.PARAMS_ERROR);
        }
        // todo 在此处将实体类和 DTO 进行转换
        Barrage barrage = new Barrage();
        BeanUtils.copyProperties(barrageEditRequest, barrage);
        // 数据校验
        barrageService.validBarrage(barrage, false);
        User loginUser = userService.getLoginUser(request);
        // 判断是否存在
        long id = barrageEditRequest.getId();
        Barrage oldBarrage = barrageService.getById(id);
        ThrowUtils.throwIf(oldBarrage == null, ErrorCode.NOT_FOUND_ERROR);
        // 仅本人或管理员可编辑
        if (!oldBarrage.getUserId().equals(loginUser.getId()) && !userService.isAdmin(loginUser)) {
            throw new BusinessException(ErrorCode.NO_AUTH_ERROR);
        }
        // 操作数据库
        boolean result = barrageService.updateById(barrage);
